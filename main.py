...
PRICE_MIN = 0.6
PRICE_MAX = 8.0
MIN_VOLUME = 300_000        # 24h quote volume threshold
...
def evaluate_symbol(sym, last_price, qvol, change_24h):
    try:
        ...
        # EMA computed but not used as a hard filter
        ...
        if vol_5m is not None:
            # reward stability above the minimum threshold (you can invert if you prefer lower volatility bonus)
            score += max(0.0, (vol_5m - 0.0004)) * 100.0
        if rsi_val is not None:
            score += max(0.0, (70.0 - min(rsi_val, 70.0))) * 0.5
        if ob_bull:
            score += 25.0

        # hard filters per your spec (EMA not required)
        if pct_1m < 0.8:
            return None
        if vol_5m is None or vol_5m < 0.0004:
            return None
        if not ob_bull:
            return None
        ...
        strong_candidate = (pct_5m >= MIN_5M_PCT and pct_1m >= 0.8 and ob_bull)
